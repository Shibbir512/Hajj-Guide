...
export const HajjMap: React.FC<HajjMapProps> = ({ locations }) => {
  return (
    <div className="h-[600px] w-full rounded-xl overflow-hidden border border-black/10 dark:border-[#22404c] z-10 relative">
      <MapContainer 
          ...
          <Marker key={loc.id} position={loc.coord} icon={customIcon}>
            <Popup className="hajj-popup max-w-sm">
              <div className="p-1 max-h-[400px] overflow-y-auto">
                <h3 className="text-lg font-serif font-bold text-[#c48c48] mb-2">{loc.name}</h3>
                {loc.dataRef ? (
                        ...
                        <ul className="space-y-2 text-xs text-gray-600">
                          {loc.dataRef.actions.slice(0, 3).map((act, i) => (
                            <li key={i} className="flex gap-2">
                              <span className="font-bold text-[#c48c48] whitespace-nowrap">[{act.type}]</span>
                              <span>{act.description}</span>
                            </li>
                          ))}
                          {loc.dataRef.actions.length > 3 && (
                            <li className="text-[#c48c48] text-center pt-1 border-t border-black/5">+ {loc.dataRef.actions.length - 3} more actions (see below)</li>
                          )}
                        </ul>
                      </div>
                    )}
                        ...
                        <div className="space-y-3">
                          {loc.dataRef.prayers.slice(0, 2).map((prayer, i) => (
                            <div key={i} className="bg-slate-50 p-2 rounded border border-black/5">
                              <div className="text-xs font-bold text-[#c48c48] mb-1">{prayer.scenario}</div>
