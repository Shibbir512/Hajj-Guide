...
export const BookmarkView: React.FC<BookmarkViewProps> = ({
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-12 py-12 md:py-20 relative z-20">
      {/* Header */}
      <div className="mb-12 border-b border-black/10 dark:border-[#22404c] pb-6">
      ...
      {hasBookmarks ? (
        <div className="grid lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-4 bg-white dark:bg-[#12242c] border border-black/10 dark:border-[#22404c] p-6 shadow-sm">
            ...
            <div className="space-y-3 mb-8">
              <button
                ...
                className={`w-full flex items-center justify-between p-4 border transition-all text-left ${
                  activeTab === 'duas'
                    ? 'border-[#c48c48] bg-[#c48c48]/5 text-gray-900 dark:text-white font-semibold'
                    : 'border-black/5 dark:border-white/5 bg-slate-50 dark:bg-[#182f3a] text-gray-600 dark:text-white/60 hover:border-[#c48c48]/35'
                }`}
              >
                <div className="flex items-center gap-3">
                  <HandHeart className="w-5 h-5 text-[#c48c48]" />
                  <span>মাসনুন দোয়া (Duas)</span>
                </div>
                <span className="bg-[#c48c48] text-white px-2.5 py-0.5 rounded-full text-xs font-sans">
              ...
              >
                <div className="flex items-center gap-3">
                  <MapPin className="w-5 h-5 text-[#c48c48]" />
            ...
            {activeTab === 'duas' ? (
              <div>
                ...
                {filteredPrayers.length === 0 ? (
                  <div className="bg-slate-50 dark:bg-[#182f3a] border border-black/10 dark:border-[#22404c] p-10 text-center">
                    ...
                    <button
                      ...
                      className="px-6 py-2.5 bg-[#c48c48] text-white hover:bg-[#a87132] text-xs font-bold uppercase transition-colors inline-flex items-center gap-2"
                    >
                  ...
                  <div className="space-y-6">
                    {filteredPrayers.map((sp, idx) => (
                      <div 
                        key={idx} 
                        className="bg-white dark:bg-[#12242c] border border-black/10 dark:border-[#22404c] p-6 md:p-8 hover:border-[#c48c48]/50 transition-colors relative group shadow-sm dark:shadow-none"
                      >
                        <div className="absolute top-0 right-0 w-16 h-16 border-t border-r border-[#c48c48] opacity-0 group-hover:opacity-20 transition-opacity"></div>
                        
                        {/* Day/Section indicator */}
                        <div className="flex flex-wrap items-center justify-between gap-3 mb-5 border-b border-black/10 dark:border-[#22404c] pb-4">
                          <div className="flex flex-col">
                            <span className="text-[10px] text-[#c48c48] font-semibold uppercase tracking-wider mb-0.5">
                              {sp.sectionDay} • {sp.sectionTitle}
                            </span>
                            <h4 className="font-serif text-[#c48c48] text-xl font-medium">
                              {sp.prayer.scenario}
                            </h4>
                          </div>
                        ...
                        <div className="space-y-5">
                          ...
                          {sp.prayer.pronunciation && (
                            <div className="bg-slate-50 dark:bg-[#182f3a] p-5 border border-black/5 dark:border-white/5">
                ...
                {filteredActions.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    {filteredActions.map((sa, idx) => (
                      <div 
                        key={idx} 
                        className="bg-white dark:bg-[#12242c] border border-black/10 dark:border-[#22404c] p-6 flex gap-4 hover:border-[#c48c48]/50 transition-colors relative group shadow-sm dark:shadow-none"
                      >
                        <div className="w-24 mt-1 flex-shrink-0 flex flex-col gap-1.5 justify-start">
                          <span className="inline-block text-center border border-[#c48c48]/40 text-[#c48c48] px-2 py-1 text-[10px] tracking-widest font-bold whitespace-nowrap">
                            {sa.action.type}
                          </span>
                        ...
                        <div className="flex-1 space-y-2">
                          <div className="flex gap-2 items-center justify-between">
                            <span className="text-xs font-serif text-[#c48c48] font-semibold flex items-center gap-1.5">
        ...
        <div className="bg-slate-50 dark:bg-[#182f3a] border border-black/10 dark:border-[#22404c] p-16 text-center max-w-2xl mx-auto rounded-lg">
          <Star className="w-16 h-16 text-[#c48c48]/30 mx-auto mb-6" />
          ...
          <p className="text-gray-500 dark:text-white/60 text-[15px] leading-relaxed mb-8 max-w-md mx-auto">
            আপনি যখন হজ্জ গাইড বা আমলগুলোর মধ্য দিয়ে যাবেন, তখন যেকোনো দোয়া বা আমলের পাশে থাকা স্টার ( <Star className="w-4 h-4 inline-block text-[#c48c48] fill-[#c48c48]/20" /> ) চিহ্নে স্পর্শ করে সহজেই নিজের প্রিয় তালিকা তৈরি করাল যেতে পারে।
          </p>
          <button
            onClick={onNavigateToGuide}
            className="px-8 py-4 bg-[#c48c48] text-white dark:text-[#0a0a0a] font-bold tracking-widest text-xs hover:bg-[#a87132] dark:hover:bg-white transition-colors uppercase inline-flex items-center gap-3"
          >
